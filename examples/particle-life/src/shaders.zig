...
pub const particle_advance = common_structs ++
    ...
    \\
    \\    if (looping_borders) {
    \\        if (particle.x < options.left) { particle.x += width; }
    \\        if (particle.x > options.right) { particle.x -= width; }
    \\        if (particle.y < options.bottom) { particle.y += height; }
    \\        if (particle.y > options.top) { particle.y -= height; }
    \\    } else {
    \\        if (particle.x < options.left) { particle.x = options.left; particle.vx *= -1.0; }
    \\        if (particle.x > options.right) { particle.x = options.right; particle.vx *= -1.0; }
    \\        if (particle.y < options.bottom) { particle.y = options.bottom; particle.vy *= -1.0; }
    \\        if (particle.y > options.top) { particle.y = options.top; particle.vy *= -1.0; }
    \\    }
    \\
