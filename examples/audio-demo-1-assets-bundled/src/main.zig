...
// Asset: embedded at compile time into the WASM binary.
// No external file serving required -- this is the zunk way.
const explode_bytes = @embedFile("assets/explode.ogg");
...
export fn init() void {
    ...
    app.setTitle("zunk audio demo");

    // Create the audio context and start decoding the embedded sound.
    // The browser decodes asynchronously; we poll isReady() each frame.
    _ = audio.init(44100);
    sfx_buffer = audio.loadFromMemory(explode_bytes);
...
export fn frame(dt: f32) void {
    ...
    canvas.fillText(ctx, "zunk audio demo", 20, 40);
    canvas.setFont(ctx, "14px monospace");
    canvas.setFillColor(ctx, dim);
    canvas.fillText(ctx, "@embedFile + Web Audio. no js. no html.", 20, 62);
    ...
    canvas.fillText(ctx, "asset: explode.ogg (41KB, embedded in WASM)", 20, 305);
