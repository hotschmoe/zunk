...
export fn frame(_: f32) void {
    ...
    drawMouseSection(w, h, mouse);
    drawKeyboardSection();
}

fn drawMouseSection(w: f32, h: f32, mouse: input.Mouse) void {
    canvas.setFillColor(ctx, white);
    ...
    canvas.beginPath(ctx);
    canvas.arc(ctx, mouse.x, mouse.y, 16, 0, tau);
    canvas.stroke(ctx);
...
fn drawKeyboardSection() void {
    canvas.setFillColor(ctx, white);
    canvas.fillText(ctx, "KEYBOARD", 20, 180);

    drawKey(ctx, 60, 195, "W", 32, input.isKeyDown(.w));
    drawKey(ctx, 20, 235, "A", 32, input.isKeyDown(.a));
    drawKey(ctx, 60, 235, "S", 32, input.isKeyDown(.s));
    drawKey(ctx, 100, 235, "D", 32, input.isKeyDown(.d));

    drawKey(ctx, 200, 195, "^", 32, input.isKeyDown(.arrow_up));
    drawKey(ctx, 160, 235, "<", 32, input.isKeyDown(.arrow_left));
    drawKey(ctx, 200, 235, "v", 32, input.isKeyDown(.arrow_down));
    drawKey(ctx, 240, 235, ">", 32, input.isKeyDown(.arrow_right));

    drawKey(ctx, 20, 280, "SPACE", 120, input.isKeyDown(.space));
    drawKey(ctx, 160, 280, "Sh", 32, input.isKeyDown(.shift));
    drawKey(ctx, 200, 280, "Ct", 32, input.isKeyDown(.ctrl));
    drawKey(ctx, 240, 280, "Es", 32, input.isKeyDown(.escape));
    ...
    for (num_keys, num_labels, 0..) |key, label, i| {
        drawKey(ctx, 20 + @as(f32, @floatFromInt(i)) * 40, 325, label, 32, input.isKeyDown(key));
    }
}
...
fn drawKey(c: canvas.Ctx2D, x: f32, y: f32, label: []const u8, key_width: f32, active: bool) void {
    canvas.setFillColor(c, if (active) green else dim);
    canvas.fillRect(c, x, y, key_width, 32);
    canvas.setFillColor(c, if (active) bg else white);
    canvas.fillText(c, label, x + key_width / 2 - 10, y + 21);
}

fn drawButton(c: canvas.Ctx2D, x: f32, y: f32, label: []const u8, active: bool) void {
    canvas.setFillColor(c, if (active) red else dim);
    canvas.beginPath(c);
    canvas.arc(c, x + 14, y + 14, 14, 0, tau);
    canvas.fill(c);
    canvas.setFillColor(c, if (active) white else canvas.Color{ .r = 140, .g = 140, .b = 140 });
