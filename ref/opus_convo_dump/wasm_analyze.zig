...
///
/// Reads:
///   - Import section (§2): function imports with module/name and type signatures
///   - Export section (§7): exported functions the developer wrote
///   - Type section (§1): function type signatures (param types + return types)
...
pub fn analyze(allocator: std.mem.Allocator, wasm: []const u8) !Analysis {
    ...
    var imports_list = std.ArrayList(Import).init(allocator);
    defer imports_list.deinit();
    var exports_list = std.ArrayList(Export).init(allocator);
    defer exports_list.deinit();
    var types_list = std.ArrayList(FuncType).init(allocator);
    ...
    result.imports = try imports_list.toOwnedSlice();
    result.exports = try exports_list.toOwnedSlice();
    result.func_types = try types_list.toOwnedSlice();
...
fn parseTypeSection(
    ...
    while (i < count and pos.* < end) : (i += 1) {
        ...
        try types.append(.{ .params = params, .returns = returns });
    }
}
        ...
        if (subsection_id == 2) {
            ...
            while (fi < func_count and pos.* < subsection_end) : (fi += 1) {
                ...
                while (li < local_count and pos.* < subsection_end) : (li += 1) {
                    ...
                    if (pos.* + nl <= wasm.len) {
                        try names.append(try allocator.dupe(u8, wasm[pos.* .. pos.* + nl]));
                        pos.* += nl;
                    }
                }
                ...
                if (func_idx < imports.len) {
                    imports[func_idx].param_names = try names.toOwnedSlice();
                } else {
