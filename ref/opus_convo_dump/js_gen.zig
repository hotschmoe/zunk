/// js_gen.zig — Generates the complete JavaScript for a Zig WASM application.
///
/// Reads the WASM analysis (imports, exports, signatures), resolves each import
/// using js_resolve, and produces a single self-contained JS file that:
///
///   1. Provides all resolved Web API implementations as WASM imports
///   2. Creates the HTML (canvas, etc.) if needed
///   3. Sets up the render loop (if the WASM exports `frame`)
///   4. Handles window resize (if the WASM exports `resize`)
///   5. Initializes input polling (if input imports are present)
///   6. Only includes scaffolding code for features actually used
///
/// The generated JS is MINIMAL — only the helpers/systems that are actually
/// imported get included. A hello-world that only uses console_log gets ~1KB of JS.
...
pub const GenOptions = struct {
    ...
    public_url: []const u8 = "/",
    /// Hashed filename of the .wasm binary
    wasm_filename: []const u8,
    /// Whether to include autoreload (dev mode)
    autoreload: bool = false,
    autoreload_port: u16 = 8080,
    /// Optional bridge.js content to merge in
...
pub const GenResult = struct {
    ...
    html: []const u8,
    /// Resolution report (for diagnostics)
    report: []const u8,
...
/// Generate complete JS + HTML for a WASM application.
pub fn generate(
    allocator: std.mem.Allocator,
    analysis: *const wa.Analysis,
    opts: GenOptions,
) !GenResult {
    // Phase 1: Resolve all imports
    ...
    // Phase 2: Generate JS
    var js = std.ArrayList(u8).init(allocator);
    defer js.deinit();
    const w = js.writer();

    // Header
    try w.writeAll("// Generated by zunk — do not edit.\n");
    try w.writeAll("// https://github.com/your-org/zunk\n\n");

    try w.writeAll("(async function() {\n'use strict';\n\n");

    // --- Scaffolding: only emit what's used ---
    ...
    // --- Import object ---
    try w.writeAll("// --- WASM import bindings ---\n");
    try w.writeAll("const env = {\n");

    for (analysis.imports, 0..) |*imp, i| {
        const r = &resolutions[i];

        // Skip non-env imports
    ...
    // Post-init: wire up things that need `exports` and `memory`
    if (needs.strings) {
    ...
    // Make exports available to handle table callbacks
    if (needs.handles) {
    ...
    if (needs.render_loop) {
        ...
        if (needs.input_system) {
            try w.writeAll("  zunkInput.flush();\n");
        }
        try w.writeAll(
    ...
    if (opts.autoreload) {
        try w.print(
            \\// --- Live reload ---
            \\const ws = new WebSocket('ws://'+location.hostname+':{d}/__ztrunk_ws');
    ...
    // Window bindings for external access
    try w.writeAll(
        \\// --- Global bindings ---
        \\window.wasmBindings = exports;
        \\window.wasmMemory = memory;
        \\dispatchEvent(new CustomEvent('ZtrunkApplicationStarted', { detail: { wasm: instance, memory } }));
        \\
    );
    ...
    defer html.deinit();
    try generateHtml(html.writer(), analysis, opts, categories_used);

    // Phase 4: Generate report
    var report = std.ArrayList(u8).init(allocator);
    defer report.deinit();
    try generateReport(report.writer(), analysis, resolutions, stub_count);

    return .{
        .js = try js.toOwnedSlice(),
        .html = try html.toOwnedSlice(),
        .report = try report.toOwnedSlice(),
    };
}
...
// ============================================================================
// Scaffolding emitters — only included if needed
// ============================================================================
...
fn emitStringHelper(w: anytype) !void {
    // Note: readStr is defined AFTER wasm instantiation (needs memory)
    // Here we just declare it as a placeholder
    try w.writeAll(
...
fn generateHtml(
    ...
    if (has_canvas and has_frame) {
        // Fullscreen canvas style for game/graphics apps
    ...
    // JS shim filename will be injected by zunk build tool
...
fn generateReport(
    ...
    try w.writeAll("=== zunk binding resolution report ===\n\n");
    try w.print("Total imports: {d}\n", .{analysis.imports.len});
    try w.print("Total exports: {d}\n", .{analysis.exports.len});
    try w.print("Unresolved (stubs): {d}\n\n", .{stub_count});

    if (stub_count > 0) {
        try w.writeAll("⚠  UNRESOLVED IMPORTS — these need a bridge.js or zunk naming convention:\n");
        for (analysis.imports, 0..) |imp, i| {
            if (resolutions[i].confidence == .stub) {
                ...
                if (analysis.getImportSignature(&imp)) |ft| {
                    ...
                    try w.print(") → ", .{});
                    if (ft.returns.len == 0) try w.print("void", .{}) else for (ft.returns, 0..) |r, j| {
                        if (j > 0) try w.print(", ", .{});
