...
pub fn Module(comptime descs: []const FuncDesc) type {
    return struct {
        ...
        pub const manifest = comptime blk: {
            break :blk serializeManifest(descs);
        };

        // Generate extern fn declarations at comptime
        // The actual callable functions are generated per-descriptor
    };
}
...
fn WrapReturnType(comptime ret: ValDesc) type {
...
export fn __zunk_string_buf_ptr() [*]u8 {
    return &string_exchange_buffer;
}

/// Get size of string exchange buffer
export fn __zunk_string_buf_len() usize {
...
export fn __zunk_invoke_callback(id: u32, a0: i32, a1: i32, a2: i32, a3: i32) void {
    if (id < MAX_CALLBACKS) {
        if (callback_table[id]) |func| {
            func(a0, a1, a2, a3);
        }
    }
}
