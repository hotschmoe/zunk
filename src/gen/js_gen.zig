...
pub fn generate(
    ...
    if (needs.render_loop) {
        ...
        // Input is flushed by WASM-side input.poll() -- no flush here,
        // otherwise per-frame sets (keysPressed/keysReleased) get wiped
    ...
    // Compute a short build fingerprint from the generated JS content.
    const js_bytes = js.items;
    const full_hash = std.hash.XxHash3.hash(0, js_bytes);
    ...
    // Patch the fingerprint into the JS header comment.
    try js.replaceRange(allocator, 0, 0, "");
    const header = try std.fmt.allocPrint(allocator, "// Generated by zunk [build:{s}] -- do not edit.\n// https://github.com/hotschmoe/zunk\n\n", .{hex[0..8]});
    defer allocator.free(header);
    // Replace the original 2-line header with the fingerprinted one.
    const old_header = "// Generated by zunk -- do not edit.\n// https://github.com/hotschmoe/zunk\n\n";
    const header_pos = std.mem.indexOf(u8, js.items, old_header) orelse 0;
    if (header_pos == 0 and std.mem.startsWith(u8, js.items, old_header)) {
        try js.replaceRange(allocator, 0, old_header.len, header);
    }
